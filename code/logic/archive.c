...
static int fossil_fpath_create_path_safe(char *dest, size_t dest_size, ccstring base_path, ccstring suffix)
{
    ...
    offset += name_len;
    fossil_sys_memory_copy(dest + offset, suffix, suffix_len);
    offset += suffix_len;
    dest[offset] = '\0';
    return 0;
}
...
static fossil_io_archive_type_t get_archive_type_from_format(ccstring format)
{
    ...
    if (fossil_io_cstring_equals(format, "zip"))
        return FOSSIL_IO_ARCHIVE_ZIP;
    if (fossil_io_cstring_equals(format, "tar"))
        return FOSSIL_IO_ARCHIVE_TAR;
    if (fossil_io_cstring_equals(format, "gz") || fossil_io_cstring_equals(format, "tar.gz"))
        return FOSSIL_IO_ARCHIVE_TARGZ;
    if (fossil_io_cstring_equals(format, "bz2") || fossil_io_cstring_equals(format, "tar.bz2"))
        return FOSSIL_IO_ARCHIVE_TARBZ2;
    if (fossil_io_cstring_equals(format, "xz") || fossil_io_cstring_equals(format, "tar.xz"))
        return FOSSIL_IO_ARCHIVE_TARXZ;
    if (fossil_io_cstring_equals(format, "7z"))
        return FOSSIL_IO_ARCHIVE_7Z;
    if (fossil_io_cstring_equals(format, "rar"))
