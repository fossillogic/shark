...
)
{
    ...
    if (!fossil_io_filesys_exists(source_file)) {
        fossil_io_fprintf(FOSSIL_STDERR, "Source file does not exist: %s\n", source_file);
        return -2;
    }
    ...
    if (fossil_io_filesys_exists(target_path)) {
        if (overwrite) {
            ...
            fossil_io_fprintf(FOSSIL_STDERR, "Target already exists: %s\n", target_path);
            return -3;
        }
    }
    ...
    if (symbolic) {
        ...
        fossil_io_fprintf(FOSSIL_STDERR, "No link type specified (symbolic or hard)\n");
        return -4;
    }
