...
void show_commands(char *app_name)
{
    ...
    fossil_io_printf("{bright_black}    --atomic            Guarantee atomic swap if supported\n");
...
bool app_entry(int argc, char **argv)
{
    ...
    for (i32 i = 1; i < argc; ++i)
    {
        ...
        {
            ccstring path1 = cnull, path2 = cnull, temp_path = cnull;
            bool force = false, interactive = false, backup = false;
            bool atomic = false, progress = false, dry_run = false, no_cross_device = false;
