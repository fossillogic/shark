...
| `search` | Find files by name or content. | `-r`, `--recursive` (include subdirs)<br>`-n`, `--name <pattern>` (filename match)<br>`-c`, `--content <pattern>` (search contents)<br>`--regex` (enable regex matching globally)<br>`--name-regex` (treat filename patterns as regex)<br>`--content-regex` (treat content patterns as regex)<br>`-i`, `--ignore-case` (case-insensitive)<br>`-p`, `--path <path>` (search within specific path) |
