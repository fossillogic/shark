...
|-------------|-----------------|-----------|
| `show` | Display files and directories. | `-a`, `--all` (show hidden)<br>`-l`, `--long` (detailed info)<br>`-h`, `--human` (human-readable sizes)<br>`-r`, `--recursive` (include subdirs)<br>`-d`, `--depth <n>` (limit recursion)<br>`--as <mode>` (format: list/tree/graph)<br>`--time` (show timestamps)<br>`-s`, `--sort <key>` (sort by: name/size/time/type asc/desc)<br>`-m`, `--match <pattern>` (filter by name/glob)<br>`--regex` (enable regex matching)<br>`--ignore-case` (case-insensitive matching)<br>`--size <filter>` (filter by size: e.g., >1MB)<br>`--min-size <size>` (minimum file size)<br>`--max-size <size>` (maximum file size)<br>`-t`, `--type <filter>` (filter by type: file/dir/link/socket/block)<br>`--mime <type>` (filter by MIME type)<br>`-x`, `--exclude <pattern>` (exclude files/directories)<br>`--exclude-dir <dir>` (exclude specific directories)<br>`--modified-before <date>` (files modified before date)<br>`--modified-after <date>` (files modified after date)<br>`--created-before <date>` (files created before date)<br>`--created-after <date>` (files created after date)<br>`--follow-symlinks` (traverse symbolic links)<br>`--color` (colored output)<br>`--icons` (display filetype icons)<br>`--absolute` (show absolute paths)<br>`--relative` (show relative paths)<br>`--json` (structured JSON output)<br>`--csv` (CSV formatted output)<br>`--count` (show totals only)<br>`--parallel` (enable multithreaded traversal)<br>`--threads <N>` (set worker thread count) |
