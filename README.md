...
| `snapshot` | Capture the current state of a file or directory. | `--file <path>` (target file)<br>`--dir <path>` (target directory)<br>`--name <label>` (snapshot label)<br>`--media` (media format output text/fson/json)<br>`--diff <other>` (compare with previous snapshot)<br>`--compress` (store snapshot compressed) |
